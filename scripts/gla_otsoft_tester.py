...
THIS FILE IS AN EXACT COPY OF THE LFCD VERSION - NO ADJUSTMENTS MADE YET!
...
def main(relfilepath=None):
    ...
    for tfpdir in [fn for fn in testfileparentdirs if os.path.isdir(fn)]:
        testtableauxfilenames = [os.path.join(tfpdir, fn) for fn in os.listdir(tfpdir) if fn == grammarfilename.replace("DraftOutput", "_test")]
        try:
...
if __name__ == "__main__":
    ...
    for secondfolder in folderswithLFCDfiles:
        ...
        for DOfile in DraftOutputfiles:
            if "Fin" in DOfile or "NSeto" in DOfile or "SSeto" in DOfile:  # True
