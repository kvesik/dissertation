...
OUTPUTS_DIR = "../sim_outs/20250317_stress_outputs"
t = "\t"
langstrings = ["latin_main", "latin_secondary"]
        ...
        with io.open(self.TESTSfilepath, "r") as tf:
            ...
            while ln != "" and "time elapsed" not in ln:
                if ln.strip() == "":
                    ...
                    if current_overtform:
                        # summarize how good this result was
                        results[current_overtform] = (current_goodresultsfreq, current_badresultsfreq)
                    # refresh info
                    current_overtform = ""
                    current_goodresultsfreq = 0
                    current_badresultsfreq = 0
                    ...
                    freq_idx = ln.index("frequency")
                    ln_head = ln[:freq_idx].strip()
                    # ln_tail = ln[freq_idx:].strip()
                    current_overtform = ln_head
                    # current_ur = removestresses(current_overtform)
                else:
                    m = re.search('\d\.\d', ln)
                    digit_idx = m.start()
                    ln_head = ln[:digit_idx].strip()
                    ln_tail = ln[digit_idx:].strip()
                    first3entries = [ln_head] + ln_tail.split()[:2]
                    # first3entries = ln.strip().split(t)[:3]  # candidate, inputfrequency, outputfrequency
                    cand = first3entries[0]
                    infreq = float(first3entries[1])
                    outfreq = float(first3entries[2])

                    iscandidateacceptablewinner = infreq > 0
                    ...
                    if iscandidateacceptablewinner:
...
def main_individual(TESTSfilepath=None):
    ...
    begoflangstring = TESTSfilename.index("latin")
    ...
    testfileparentparentdir = os.path.join("..", "sim_ins", "20250317 on - stress")
        ...
        with io.open(grammar.TESTSfilepath, "r") as tf:
            ...
            while tf_ln != "" and "time elapsed" not in tf_ln:
                if tf_ln.strip() == "":
                    ...
                    if current_overtform:
                        wf.write("frequency of good test results: " + str(testresults[current_overtform][0]) + "\n")
                        wf.write("frequency of bad test results: " + str(testresults[current_overtform][1]) + "\n")
                        wf.write("\n")
                    current_overtform = ""
                else:
                    if "frequency" in tf_ln:
                        freq_idx = tf_ln.index("frequency")
                        ln_head = tf_ln[:freq_idx].strip()
                        # ln_tail = ln[freq_idx:].strip()
                        current_overtform = ln_head
                        # current_ur = removestresses(current_overtform)
                    wf.write(tf_ln)
...
def main_overall_onefolder(analysisfilepath=None):
    ...
    begoflangstring = analysisfilename.index("OTSoft-") + 7
