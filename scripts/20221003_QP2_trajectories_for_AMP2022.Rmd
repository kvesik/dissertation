...
# coltitles_starB_noAgr <- c("trialnum","generated","heard","star_F_change","star_F","star_B_change","star_B","star_7_change","star_7","star_e_change","star_e","IdBkSyl1_change","IdBkSyl1","IdBk_change","IdBk","AgrBk_change","AgrBk","GMHF_change","GMHF","GMH7_change","GMH7","GMHe_change","GMHe")

prepare_data <- function(filename, hasstarB) {  #}, showAgr) {
  ...
  #   df$trialnum[1] <- 0
  #   
  # } else if (hasstarB && ! showAgr) {
  if (hasstarB) {  # and we don't want to show Agree(Bk)
    colnames(df) <- coltitles_starB
    df <-
      df %>%
      ### select(trialnum, star_F, star_B, star_7, star_e, IdBkSyl1, IdBk, GMHF, GMHB, GMH7, GMHe)
      ### select(trialnum, IdBkSyl1, star_F, star_B, IdBk, star_7, star_e, GMHF, GMHB, GMH7, GMHe) # %>%
      select(trialnum, IdBkSyl1, star_e, star_F, star_7, star_B, GMHe, GMHF, GMH7, GMHB, IdBk)
      # filter(trialnum < 50000)
    
    # fill in missing data and adjust trial number for initial state
    df <- 
      df %>% 
      ### fill(trialnum,star_F,star_B,star_7,star_e,IdBkSyl1,IdBk,GMHF,GMHB,GMH7,GMHe) %>%
      ### fill(trialnum,IdBkSyl1,star_F,star_B,IdBk,star_7,star_e,GMHF,GMHB,GMH7,GMHe) %>%
      ### fill(trialnum,IdBkSyl1,star_F,star_B,IdBk,star_7,star_e,GMHF,GMHB,GMH7,GMHe, .direction = "up")
      fill(trialnum,IdBkSyl1, star_e, star_F, star_7, star_B, GMHe, GMHF, GMH7, GMHB, IdBk) %>%
      fill(trialnum,IdBkSyl1, star_e, star_F, star_7, star_B, GMHe, GMHF, GMH7, GMHB, IdBk, .direction = "up")
    df$trialnum[1] <- 0
  
  }
  # } else if (showAgr && ! hasstarB) {
  ...
  # } else if (hasstarB && ! showAgr) {
  if (hasstarB) {  # and we don't want to show Agree(Bk)
    # constraint_levels <- c("star_F", "star_B", "star_7", "star_e", "IdBkSyl1", "IdBk", "GMHF", "GMHB", "GMH7", "GMHe")
    # constraint_labels <- c("*F", "*B", "*ɤ", "*e", "Id-Syl1(Bk)", "Id(Bk)", "VH(F)", "VH(B)", "VH(ɤ)", "VH(e)")
    # constraint_levels <- c("star_F","star_B","star_7","star_e","IdBkSyl1","IdBk","GMHF","GMHB","GMH7","GMHe")
    # constraint_labels <- c("*F","*B","*ɤ","*e","Id-Syl1(Bk)","Id(Bk)","VH(F)","VH(B)","VH(ɤ)","VH(e)")
    # constraint_levels <- c("IdBkSyl1","star_F","star_B","IdBk","star_7","star_e","GMHF","GMHB","GMH7","GMHe")
    # constraint_labels <- c("Id-Syl1(Bk)","*F","*B","Id(Bk)","*ɤ","*e","VH(F)","VH(B)","VH(ɤ)","VH(e)")
    constraint_levels <- c("IdBkSyl1","star_e","star_F","star_7","star_B","GMHe","GMHF","GMH7","GMHB","IdBk")
    # constraint_labels <- c("Id-Syl1(Bk)","*e","*æ,*ø,*y","*ɤ","*ɑ,*o,*u","VH(e)","VH(æ,ø,y)","VH(ɤ)","VH(ɑ,o,u)", "Id(Bk)")
    constraint_labels <- c("Id-Syl1(Bk)","*e","*F","*ɤ","*B","VH(e)","VH(F)","VH(ɤ)","VH(B)", "Id(Bk)")
    # constraint_labels_parsed <- parse(text=constraint_labels)
  }
...
plot_trajectories <- function(df, lang, trialsperbatch, experimentnum) {
  ...
  upperlimit <- if (str_detect(experimentnum, "without")) 140 else 110
  ...
  trajectoryplot <-
    ...
    geom_line(aes(color=constraint, size=constraint), show.legend=TRUE) + # size=0.6, 
    scale_size_manual( values = c(1.1, 0.4, 0.4, 0.4, 0.4, 1.1, 1.1, 0.4, 0.4, 1.1) ) +
    ggtitle(paste0(lang, " learning trajectories over 4x", trialsperbatch, " trials (", experimentnum, ")")) +
    ...
    scale_y_continuous(limits=c(upperlimit-60,upperlimit), breaks=seq(from=0, to=upperlimit, by=10))
...
extract_langtrialsexpnum <- function(filename) {
  ...
  if (expnumstring == "K407p") {
    expnumwords <- "without CEDRA" # MCRR"
  } else {
    expnumwords <- "with CEDRA" # MCRR"
  }
  hasstarB <- substr(expnumstring, 2, 2) == "4"
  return(c(langstring, trialsstring, expnumwords, hasstarB))
}
...
makeplot <- function(filename) {
  ...
  hasstarB <- info[4]
  # showAgr <- FALSE
  df <- prepare_data(filename, hasstarB)  #, showAgr)
  plot_trajectories(df, lang, trials, expnumwds)
}
...
historyfiles <- c(
  ...
  "K407p_OT_Soft_KE_iFBfb_forGLA_cat_starBQP2cons_no2xcounts_HISTORY50000_forR.txt"
